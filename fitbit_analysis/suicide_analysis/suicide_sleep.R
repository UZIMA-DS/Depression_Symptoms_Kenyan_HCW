...
suicide_sleep %>%
  ...
  labs(title="Average sleep",
       subtitle = "Participants reported S_symptoms",
       x='',
       y='Time in Minutes')+
  theme_bw()
...
png("S_MinutesAsleep.png", 600, 400)
...
png("S_REM_summary2023.png", 600, 400)
grid.draw(avg_summary_table)
dev.off()
...
png("S_leveldeep_summary.png", 600, 400)
grid.draw(avg_summary_table)
dev.off()
